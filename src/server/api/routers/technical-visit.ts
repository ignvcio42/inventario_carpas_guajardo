    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await ctx.db.notification.createMany({
        data: allUsers.map((user) => ({
          ...
          message: `${ctx.session.user.name || "Un usuario"} ha programado una visita para ${input.nombreCliente} el ${new Date(input.fechaVisita).toLocaleDateString("es-CL")}`,
          userId: user.id,
  ...
  update: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (estado && oldVisit && oldVisit.estado !== estado) {
        await ctx.db.notification.createMany({
          data: allUsers.map((user) => ({
            title: "Estado de visita técnica actualizado",
            message: `${ctx.session.user.name || "Un usuario"} cambió el estado de la visita de ${updatedVisit.nombreCliente} de ${oldVisit.estado} a ${estado}`,
            userId: user.id,
            actionBy: ctx.session.user.id,
            actionByName: ctx.session.user.name || "Usuario",
          })),
      ...
      // Si se editó cualquier otro campo (no solo el estado)
        ...
        await ctx.db.notification.createMany({
          data: allUsers.map((user) => ({
            ...
            message: `${ctx.session.user.name || "Un usuario"} ha editado la visita técnica de ${updatedVisit.nombreCliente}`,
            userId: user.id,
            actionBy: ctx.session.user.id,
            actionByName: ctx.session.user.name || "Usuario",
          })),
